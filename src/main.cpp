...
#if defined(NDEBUG)
# error "Spartacuscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb3f712f94688c93537daa894f8a4cad1f47583f953ab3baebb0c44ac5b87264e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Spartacuscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Spartacuscoin Signed Message:\n";
...
{
    // Spartacuscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 100000); // Spartacuscoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Spartacuscoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Spartacuscoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Spartacuscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Spartacuscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x399ee9190a5488d2a466bb0b97a40795a04b5e2fb6ffc34566e88c4fdad08401");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Vile acid attacker who scarred Kate Piper for life";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20  * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04bbb045f9fe1b2adc04c2bad1509c51deb795916e5c9bbb4581394aeddeb69f2da6ad21d705d321c466df6ac68e1693563e6d9934b0f296d68fac34995f9b1461") << OP_CHECKSIG;                                                          
        ...
        block.nVersion = 1;
        block.nTime    = 1520695071;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 26744;

        if (fTestNet)
        {
            block.nTime    = 1520695059;
            block.nNonce   = 1423014;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xa7f626c8d64aa9ca14326ab32025fe7f10faa9302c79ace80df0bc887c26e323"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc1, 0xbd, 0xde }; // Spartacuscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SpartacuscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SpartacuscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SpartacuscoinMiner(CWallet *pwallet)
{
    printf("SpartacuscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("spartacuscoin-miner");
    ...
    try { loop {
        ...
        printf("Running SpartacuscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SpartacuscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SpartacuscoinMiner, pwallet));
}
